#!/bin/bash
# Original: https://github.com/AkihiroSuda/containerized-systemd/blob/master/docker-entrypoint.sh
set -e
container=docker
export container
...
Requires=docker-entrypoint.service systemd-logind.service systemd-user-sessions.service sshd.service
EOF
...
# custom hacks
yum install -y openssh-server python3 sudo iproute
systemctl enable sshd.service
useradd hivemind
mkdir /home/hivemind/.ssh
echo 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAID4STEKdHMw1cyatC5uPJRzjCvjwX6Dh7hMB6gCMjinq ansible' > /home/hivemind/.ssh/authorized_keys
chown -R hivemind:hivemind /home/hivemind/.ssh
echo 'hivemind    ALL=(ALL)       NOPASSWD: ALL' > /etc/sudoers.d/hivemind
# end custom hacks
